...
export default async function getWeatherData(location, units) {
  ...
  try {
    ...
    if (!response.ok) {
      statusError = true;
      throw new Error(`The server responded with a status of ${response.status}. The location entered might not be available.`);
    }
    ...
    else throw new Error(`${error.message}: Ensure that your device is connected to the internet.`);
  }
}
